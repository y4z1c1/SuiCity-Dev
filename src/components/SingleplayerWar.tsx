...
const SingleplayerWar = ({
    ...
    useEffect(() => {
        const checkLastWarTime = () => {
            ...
            const diffInMs = lastWarTime.getTime() + 6 * 60 * 60 * 1000 - currentTime.getTime(); // 6 hours in milliseconds
    ...
    const startWar = useCallback(async () => {
        ...
        try {
            ...
            transactionBlock.moveCall({
                target: `${ADDRESSES.PACKAGE}::nft::${isShieldActive ? "simulate_singleplayer_war_with_sui" : "simulate_singleplayer_war"}`,
                arguments: [
                    ...
                    ...(isShieldActive ? [coinWithBalance({ balance: 200000000 }),] : []), // Additional SUI cost if shield is on
                ...
                },
                {
                    onSuccess: async (result) => {
                        ...
                        if (event) {
                            ...
                            const videoPath = `/videos/${videoType}${isWin ? "w" : "l"}.mp4`;
                            setVideoSource(videoPath);

                            setIsWarInProgress(true);
                            setIsFading(true); // Start the fade effect
                            setTimeout(createAnimation, 500); // Start the animation
                            setTimeout(() => {
                            ...
                            }, 1500); // Toggle between red and darkred every 500ms
    ...
    return (
        <>
            ...
            {showModeSelection && (
                <div className="war-modal">
                    ...
                    <div className="war-cards-container">
                        {[
                            { image: "e4.png", cityName: "Jack's City", population: "350k", power: 100, reward: "1000 $SITY" },
                            { image: "e1.png", cityName: "Fred's City", population: "350k", power: 700, reward: "7000 $SITY" },
                            { image: "e2.png", cityName: "Jonas's City", population: "350k", power: 2500, reward: "25000 $SITY" },

                            { image: "e0.png", cityName: "Paul's City", population: "2.4M", power: 6000, reward: "60000 $SITY" },
                            { image: "e3.png", cityName: "Theodor's City", population: "16M", power: 10000, reward: "100000 $SITY" }
                        ].map((enemy, index) => (
                            <div
                            ...
                            >
                                <img src={`/public/${enemy.image}`} alt={`${enemy.cityName} image`} className="enemy-image" />
                    ...
                    <div className="war-buttons">
                        ...
                        <button
                            ...
                            onClick={startWar}
                            disabled={loading || opponentPower === null}
                        >
                            {loading ? "Preparing for War..." : `Start War ${isShieldActive ? "for 0.2 SUI" : ""}`}
                        </button>
            ...
            {isWarInProgress && (
                <div className={`war-overlay ${isFading ? 'fading' : ''} ${showWarVideo ? 'visible' : ''} ${isResultShown ? 'background-none' : ''}`}>
                    {/* War Video */}
                    {videoStarted && (
